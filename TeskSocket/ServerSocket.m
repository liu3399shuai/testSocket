...
- (void)shareScreen
{
    ...
    {
        [s writeData:[ServerSocket stringToData:@"sharescreen"] withTimeout:-1 tag:0];
    }
}
- (void)stopShareScreen
{
    for (AsyncSocket * s in connectedSockets)
    {
        [s writeData:[ServerSocket stringToData:@"stopsharescreen"] withTimeout:-1 tag:0];
    }
}
...
- (void)writeScreen:(NSData *)imageData
{
    for (AsyncSocket * s in connectedSockets)
    {
        [s writeData:imageData withTimeout:-1 tag:0];
    }
}
...
- (void)stopWriteScreen
{
    for (AsyncSocket * s in connectedSockets)
    {
        [result appendString:@"stopWriteScreen\n"];
        [s writeData:[ServerSocket stringToData:@"stopWriteScreen"] withTimeout:-1 tag:0];
    }
}

- (void)startShareScreen
{
    [[NSNotificationCenter defaultCenter] postNotificationName:START_SHARE_NOTIFICATION object:nil];
}
...
- (void)sendMessage
{
    for (AsyncSocket * s in connectedSockets)
    {
        [s writeData:[ServerSocket stringToData:@"西点军校在哪个国家 ？"] withTimeout:-1 tag:0];
    }
}
...
- (void)startListen
{
    ...
    {
        NSError *error = nil;
        [listenSocket acceptOnPort:LISTEN_PORT error:&error];
...
- (void)onSocket:(AsyncSocket *)sock didConnectToHost:(NSString *)host port:(UInt16)port
{
    ...
    [sock writeData:[ServerSocket stringToData:@"恭喜您，已连接服务器！"] withTimeout:-1 tag:0];
